...
======*/
resource "aws_cloudwatch_log_group" "pdi" {
  name = "pdi"
...
Repository
=====*/
resource "aws_ecr_repository" "pdi" {
...
data "template_file" "web_task" {
  ...
  vars = {
    image           = aws_ecr_repository.pdi.repository_url
    ...
    log_group       = aws_cloudwatch_log_group.pdi.name
  }
}
...
resource "aws_alb" "alb_pdi" {
  name            = "${var.environment}-alb-pdi"
...
resource "aws_alb_listener" "pdi" {
  load_balancer_arn = aws_alb.alb_pdi.arn
  port              = "80"
  protocol          = "HTTP"
...
resource "aws_cloudwatch_metric_alarm" "service_cpu_high" {
  alarm_name          = "${var.environment}_pdi_web_cpu_utilization_high"
